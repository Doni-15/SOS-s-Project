...
export const CartDrawer = ({ isOpen, onClose, disableOverlay = false, sidebar = false }) => {
  ...
  const renderContent = (closeable = true) => (
    <>
      {/* Header */}
      <div className="sticky top-0 bg-blue-600 text-white p-4 flex items-center justify-between">
        ...
        {closeable && (
          <button onClick={handleClose} className="p-2 hover:bg-blue-500 rounded">
            <X size={24} />
          </button>
        )}
      </div>
  ...
  // Sidebar variant for desktop: non-fixed, visible on lg and up
  if (sidebar) {
    return (
      <div className="hidden lg:flex lg:w-96 lg:flex-col">
        <div className="bg-white shadow-inner h-[calc(100vh-4rem)] overflow-y-auto flex flex-col">
          {renderContent(false)}
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col h-full">{renderContent(true)}</div>
      </div>
    </>
  );
};
